...
Usage:
    DYNAMODB_ENDPOINT=http://localhost:8000 python -m llms_txt_worker.dev_runner

Replaces SQS + Lambda for local development. The Next.js dev server creates
PENDING jobs in DynamoDB Local; this script picks them up and runs the full
crawl-to-llms.txt pipeline.
"""
...
def main() -> None:
    ...
    # (SSO, instance metadata). DynamoDB Local accepts any key pair.
    os.environ.setdefault("AWS_ACCESS_KEY_ID", "local")
    os.environ.setdefault("AWS_SECRET_ACCESS_KEY", "local")

    from llms_txt_worker.handler import _process_job
    from llms_txt_worker.persistence.repository import scan_pending_jobs
