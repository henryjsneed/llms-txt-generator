...
class TestSkipBareUrls:
    ...
    def test_skips_page_with_no_title_even_if_description(self):
        """URL-as-title entries are useless in llms.txt — skip them."""
        sections = [
            GroupedSection(
                name="Docs",
                pages=[PageMetadata(url="https://ex.com/docs", title="Docs")],
        ...
        result = generate_llms_txt("S", "", sections)
        assert_spec_compliant(result)
        assert "Full reference docs" not in result
        assert "ex.com/thing" not in result
...
class TestOptionalSection:
    """All optional entries should appear in the output."""

    def test_all_optional_entries_included(self):
        pages = [
            PageMetadata(url=f"https://ex.com/page-{i}", title=f"Page {i}")
            for i in range(30)
        ]
        sections = [
            GroupedSection(
            ...
            GroupedSection(name="Misc", pages=pages, is_optional=True),
        ]
        result = generate_llms_txt("S", "D", sections)
        assert_spec_compliant(result)
        optional_items = [ln for ln in result.split("\n") if ln.startswith("- ")]
        assert len(optional_items) == 1 + 30
...
class TestDescriptionTruncation:
    """Long descriptions are truncated at sentence boundaries."""
    ...
    def test_truncates_at_sentence_boundary(self):
        long_desc = (
            ...
            "Second sentence with more detail that pushes past the limit. "
            "Third sentence that should definitely not appear because it is "
            "far beyond the reasonable description length for a link entry."
        )
        sections = [
            GroupedSection(
                name="Docs",
        ...
        result = generate_llms_txt("S", "", sections)
        assert_spec_compliant(result)
        assert "First sentence about the topic." in result
        assert "Third sentence" not in result

    def test_does_not_truncate_at_abbreviations(self):
        desc = (
            "Dr. Sanjay Gupta is on a mission to help us approach our new "
            "normal mindfully, finding a balance between self-care and "
            "productivity, and talking to doctors and researchers about the "
            "surprising science behind how we thrive."
        )
        sections = [
            GroupedSection(
                name="Docs",
        ...
        assert desc_part != "Dr."
        assert len(desc_part) > 20

    def test_truncates_at_word_boundary_when_no_sentence_end(self):
        no_period = "A " + "very " * 50 + "long description without sentence endings"
        sections = [
            GroupedSection(
                name="Docs",
                pages=[
        ...
        assert len(desc_part) <= 210  # some tolerance for the ellipsis


class TestBoilerplateDescriptionFiltering:
    """Descriptions repeated across many pages are treated as boilerplate."""

    def test_filters_description_appearing_three_plus_times(self):
        tagline = "Production-Grade Container Orchestration"
        pages = [
            PageMetadata(
                url=f"https://ex.com/releases/{v}",
                title=f"Release {v}",
                description=tagline,
            )
            for v in ("1.10", "1.11", "1.12")
        ]
        sections = [GroupedSection(name="Releases", pages=pages)]
        result = generate_llms_txt("Kubernetes", "", sections)
        assert_spec_compliant(result)
        assert tagline not in result
        assert "[Release 1.10]" in result

    def test_keeps_description_appearing_twice(self):
        desc = "Some repeated description"
        pages = [
            PageMetadata(url=f"https://ex.com/p{i}", title=f"P{i}", description=desc)
            for i in range(2)
        ]
        sections = [GroupedSection(name="Docs", pages=pages)]
        result = generate_llms_txt("S", "", sections)
        assert desc in result

    def test_filters_template_descriptions_with_shared_prefix(self):
        """Descriptions differing only by a category word should be boilerplate."""
        categories = ["beauty", "fashion", "home", "electronics"]
        pages = [
            PageMetadata(
                url=f"https://ex.com/shop/{cat}",
                title=cat.title(),
                description=f"Underscored is your trusted source for the latest {cat} trends, how-to's, and product reviews.",
            )
            for cat in categories
        ]
        sections = [GroupedSection(name="Shop", pages=pages)]
        result = generate_llms_txt("CNN", "", sections)
        assert_spec_compliant(result)
        assert "trusted source" not in result
        for cat in categories:
            assert f"[{cat.title()}]" in result

    def test_filters_description_matching_site_summary(self):
        summary = "Kubernetes is an open source container orchestration engine."
        sections = [
            GroupedSection(
                name="Docs",
                pages=[
        ...
        result = generate_llms_txt("Kubernetes", summary, sections)
        assert_spec_compliant(result)
        line = next(ln for ln in result.split("\n") if ln.startswith("- "))
        assert ": " not in line  # description filtered


class TestTitlePrefixStripping:
    def test_strips_matching_site_prefix(self):
        sections = [
            GroupedSection(
                name="Docs",
                pages=[
                    PageMetadata(url="https://ex.com/books", title="Amazon.com: Books"),
                ],
            ),
        ]
        result = generate_llms_txt("Amazon.com", "", sections)
        assert_spec_compliant(result)
        assert "[Books]" in result
        assert "Amazon.com:" not in result

    def test_keeps_prefix_when_not_site_title(self):
        sections = [
            GroupedSection(
                name="Docs",
                pages=[
                    PageMetadata(url="https://ex.com/x", title="CNN: Breaking Story"),
                ],
            ),
        ]
        result = generate_llms_txt("Amazon.com", "", sections)
        assert "CNN: Breaking Story" in result

    def test_includes_page_description(self):
        sections = [
            GroupedSection(
                name="Docs",
                pages=[
                    PageMetadata(
                        url="https://ex.com/x",
                        title="Fire TV",
                        description="Stream movies, shows, and live TV with Fire TV.",
                    ),
                ],
            ),
        ]
        result = generate_llms_txt("Amazon.com", "", sections)
        assert_spec_compliant(result)
        assert "[Fire TV]" in result
        assert "Stream movies" in result
