...
export default function JobProgress({ jobId }: { jobId: string }) {
    ...
    return (
      <div className="mx-auto max-w-3xl py-12">
        <div className="rounded-xl border border-red-200 bg-red-50/70 p-5 dark:border-red-900/40 dark:bg-red-950/20">
          ...
          <p className="mt-2 text-base leading-7 text-red-900 dark:text-red-100 break-words whitespace-pre-wrap">
            {error}
          </p>
        </div>
  ...
  if (job.status === "FAILED") {
    return (
      <div className="mx-auto max-w-3xl py-12">
        <div className="inline-flex items-center gap-2 text-red-600 text-xl font-semibold dark:text-red-400">
          <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          Generation failed
        </div>
        <div className="mt-3 rounded-xl border border-red-200 bg-red-50/70 p-5 dark:border-red-900/40 dark:bg-red-950/20">
          <p className="text-sm font-semibold uppercase tracking-wide text-red-700 dark:text-red-300">
            Reason
          </p>
          <p className="mt-2 text-base leading-7 text-red-900 dark:text-red-100 break-words whitespace-pre-wrap">
